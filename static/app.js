...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalKrita = setInterval(function(){
            pintarBarra(krita, 15, 3, intervalKrita);
        },100);
        const intervalSql = setInterval(function(){
            pintarBarra(sql, 14, 4, intervalSql);
        },100);
        const intervalDibujo = setInterval(function(){
            pintarBarra(dibujo, 16, 5, intervalDibujo);
        },100);
    }
}
...
function pintarBarra(id_barra, cantidad, indice, interval){
    ...
    if(x < cantidad){
        ...
        elementos[x].style.backgroundColor = "#b80065";
    }else{
